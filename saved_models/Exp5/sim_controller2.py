...
class SimController(object):
	...
	def getReward(self, new_state, robot_allies, robot_opponents, ball):
		...
		# reward += 10/(0.05 + 0.01*diff_direction) + 5/(0.05 + 0.01*diff_distance)#/(0.1*self.times_since_restart)
		reward += 4*(MAX_DIST - diff_distance*4)/143
		reward += 1*(math.pi - diff_direction*4)/math.pi
		# print('reward',reward)
		# reward +=reward1
		goal_x = MAX_DIST/2
		...
		reward += ball_x*0.3
		ball_y = abs(ball.body.position[1])
		reward += -ball_y*0.2
	...
	def compute(self, robot_allies, robot_opponents, ball):
		...
		if(len(self.batch[0]) >= BUFFER_SIZE or self.isGoal):
			...
			print('restarting...')
			self.restart = True
			print(eny_print,self.episodes, ' out of ', MAX_EPISODES*10)
		...
		if(self.episodes > MAX_EPISODES*10):#self.times > MAX_FRAMES*10):
			...
			plotter.plotRewards(self.mean_rewards, figname=file_name)
			plotter.saveCSV(self.mean_rewards, figname=file_name)
			exit()
