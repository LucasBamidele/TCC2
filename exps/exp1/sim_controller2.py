...
class SimController(object):
	...
	def __init__(self):
		...
		if(only_play or load_model):
			self.model = nn.neural_net_model2_1(NUMBER_OF_PLAYERS, model_name)
		else :
			self.model = nn.neural_net_model2_1(NUMBER_OF_PLAYERS)
		self.reward = {
			'goal': GOAL_REWARD,
			'pass': PASS_REWARD,
	...
	def playerHitBall(self, robot_allies, robot_opponents, ball):
		if(robot_allies[0].body.userData == ball.body):
			...
			if(distance_cool(robot_allies[0].body.angle, angle_between_bodies(robot_allies[0].body, ball.body)) < math.pi/2):
				return True
		return False
